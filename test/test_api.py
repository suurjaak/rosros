  ...
  frame_id='%s'
    ...
    def test_temporals(self):
        ...
        with self.subTest(NAME(func)):
            ...
            val = api.make_time(123456789, 987654321)
            tval = func(val)
            self.assertTrue(api.is_ros_time(tval), ERR(func))
            self.assertEqual(api.to_sec_nsec(tval), api.to_sec_nsec(val), ERR(func))

            val = api.time_message(api.make_time(123456789, 987654321))
            tval = func(val)
            self.assertTrue(api.is_ros_time(tval), ERR(func))
            self.assertEqual(api.to_sec_nsec(tval), api.to_sec_nsec(val), ERR(func))
        ...
        with self.subTest(NAME(func)):
            ...
            val = api.time_message(api.make_duration(123456789, 987654321))
            tval = func(val)
            self.assertTrue(api.is_ros_time(tval), ERR(func))
            self.assertEqual(api.to_sec_nsec(tval), api.to_sec_nsec(val), ERR(func))
